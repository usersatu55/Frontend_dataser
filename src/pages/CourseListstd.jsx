...
function CourseListstd() {
  ...
  const handleCancelEnrollment = async (course_code) => {
    ...
    const confirmCancel = window.confirm(`คุณต้องการยกเลิกการลงทะเบียนในวิชา ${course_code} หรือไม่?`);
    if (confirmCancel) {
      try {
              ...
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-center">ยกเลิกการลงทะเบียน</th>
                  </tr>
                </thead>
                <tbody>
                  {courses.map((course, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 w-48">
                        <button
                          onClick={() => navigate(`/status/${course.course_code}`)}
                          type="button"
                      ...
                      <td className="px-6 py-4 w-48">
                        <button
                          onClick={() => handleCancelEnrollment(course.course_code)}
                          type="button"
